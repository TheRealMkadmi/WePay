...
{
    $setting = App\Models\Setting::where(['name' => 'Code_inst'])->first(['value']);
    return $setting->value;
}
...
function sendSMSwithWbm($wbmCredentials, $to, $message)
{

    $curl = curl_init();

    $login        = $wbmCredentials['login'];
    $password     = $wbmCredentials['password'];
    $compte       = $wbmCredentials['compte'];
    $auth_token   = $wbmCredentials['auth_token'];

    curl_setopt_array($curl, array(
    CURLOPT_URL => 'https://wbm.tn/wbmonitor/send/webapi/v2/send_ack.php',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    ...
    // CURLOPT_POSTFIELDS => 'login=TL01&pass=bjjUde3qZOE&compte=Tledger&op=1&dest_num='.$to.'&msg='.urlencode($message).'&type=0&auto_detect=1&dt=01%2F04%2F2021&hr=15&mn=38&label=TLEDGER&ref=test%20Tledger%20from%20postman&res_type=JSON',
    CURLOPT_POSTFIELDS => 'login='.$login.'&pass='.$password.'&compte='.$compte.'&op=1&dest_num='.$to.'&msg='.urlencode($message).'&type=0&auto_detect=1&dt=01%2F04%2F2021&hr=15&mn=38&label=TLEDGER&ref=test%20Tledger%20from%20postman&res_type=JSON',
  
    CURLOPT_HTTPHEADER => array(
    'Content-Type: application/x-www-form-urlencoded',
    // 'Authorization: Basic VGxlZGdlcjpNd2lsc2thVzRPNw=='
...
function sendSMS($to, $message)
{
    ...
    {   
        ...
        if (count($smsCredentials) > 0)
        {
            ...
            else if ($smsConfig->type == 'twilio'){
                sendSMSwithTwilio($smsCredentials, $to, $message);

            }
            else if ($smsConfig->type == 'wbm'){
                // sendSMSwithWbm($to, $message);
                sendSMSwithWbm($smsCredentials, $to, $message);
        ...
        {
            if ($smsConfig->type == 'nexmo')
	@@ -762,7 +794,7 @@ function sendSMS($to, $message)
            {
                sendSMSwithTwilio($smsCredentials, $to, $message);
            }
        } */
...
{
    if (isset($setting) && ($setting['has_captcha'] == 'login' || $setting['has_captcha'] == 'registration' || $setting['has_captcha'] == 'login_and_registration')) {
...
function getAdminPanelAccessIPs()
{
    $general = App\Models\Setting::where(['type' => 'admin_security'])->get()->toArray();
