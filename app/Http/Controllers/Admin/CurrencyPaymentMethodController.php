...
class CurrencyPaymentMethodController extends Controller
{
    ...
    public function updatePaymentMethodCredentials(Request $request)
    {
        ...
        if (empty($currencyPaymentMethod))
        {
            ...
            elseif (!empty($request->clictopay))
            {
                $currencyPaymentMethod->method_data = json_encode($request->clictopay);

                if (isset($request->clictopay_status) && $request->clictopay_status == 'Active')
                {   
                    $currencyPaymentMethod->activated_for = json_encode(['deposit' => '']);
                }
                else
                ...
                $currencyPaymentMethod->processing_time = "";

            }
            elseif (!empty($request->CompteStb))
            {
                $currencyPaymentMethod->method_data = json_encode($request->CompteStb);

                if (isset($request->CompteStb_status) && $request->CompteStb_status == 'Active')
                {   
                    $currencyPaymentMethod->activated_for = json_encode(['deposit' => '']);
                }
                else
            ...
            elseif (!empty($request->CarteStb))
            {
                $currencyPaymentMethod->method_data = json_encode($request->CarteStb);

                if (isset($request->CarteStb_status) && $request->CarteStb_status == 'Active')
                {   
                    $currencyPaymentMethod->activated_for = json_encode(['deposit' => '']);
                }
                else
