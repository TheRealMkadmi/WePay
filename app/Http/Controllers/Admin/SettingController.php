...
class SettingController extends Controller
{
    ...
    public function general(Request $request)
    {
        ...
        else if ($request->isMethod('post'))
        {
            ...
            else
            {
                ...
                Setting::where(['name' => 'Code_inst'])->update(['value' => $request->Code_inst]);
    ...
    public function adminSecuritySettings(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            // Admin 2fa Enabled/Disabed
            // Setting::where(['name' => 'admin_2fa', 'type' => 'admin_security'])->update(['value' => $request->admin_2fa]);
            ...
            if(!empty($request->get('admin_url_prefix'))) {
                ...
                Preference::where(['field' => 'admin_url_prefix'])
                    ->update(['value' => $slug]);
        ...
        $settigs        = Setting::where('type', 'admin_security')->get()->toArray();
        $data['result'] = $this->helper->key_value('name', 'value', $settigs);
    ...
    public function sms(Request $request, $type)
    {
        ...
        if (!$request->isMethod('post'))
        {
            ...
            {
                $data['wbm']      = $wbm      = SmsConfig::where(['type' => $type])->first();
                $data['credentials'] = $credentials = json_decode($wbm->credentials);
                return view('admin.settings.sms.wbm', $data);
            }
        }
        else
        {
            if ($type == 'twilio')
            {
                ...
                if ($twilioSmsConfig && (($request->status == 'Active') || ($request->status == 'Inactive')))
                {
                    ...
                    if ($twilioSmsConfig->status == 'Active')
                    {
                        ...
                        $wbmSmsConfig         = SmsConfig::where(['type' => 'wbm'])->first(['id', 'status']);
                        $wbmSmsConfig->status = 'Inactive';
                        $wbmSmsConfig->save();
                    }
            ...
            else if ($type == 'wbm')
            {
                ...
                $fieldNames = array(
                    'name'                               => 'Name',
                    'wbm.login'                          => 'Wbm Key',
                    'wbm.password'                       => 'Wbm Password',
                    'wbm.compte'                         => 'Wbm Compte',
                    'wbm.auth_token'                     => 'Wbm Authentication Token',
                    'status'                             => 'Status',
                );
                $validator = Validator::make($request->all(), $rules);
                $validator->setAttributeNames($fieldNames);
                ...
                //
                $wbmSmsConfig = SmsConfig::where(['type' => base64_decode($request->type)])->first();
                
                if ($wbmSmsConfig && (($request->status == 'Active') || ($request->status == 'Inactive')))
                {
                    $wbmSmsConfig->credentials = json_encode($request->wbm);
                   
                    $wbmSmsConfig->status      = $request->status == 'Active' ? 'Active' : 'Inactive';
                    $wbmSmsConfig->save();

                    if ($wbmSmsConfig->status == 'Active')
                    {
                        $nexmoSmsConfig  = SmsConfig::where(['type' => 'nexmo'])->first(['id', 'status']);
                        $nexmoSmsConfig->status = 'Inactive';
                        $nexmoSmsConfig->save();
                    ...
                    $this->helper->one_time_message('success', 'Wbm SMS settings updated successfully!');
                    return redirect(Config::get('adminPrefix').'/settings/sms/wbm');
                }
            }
        }
    }
    ...
    public function payment_methods(Request $request)
    {
        ...
        {
            $data['menu'] = 'settings';
            $data['settings_menu'] = 'payment_methods';
        ...
        else if ($request->gateway == 'paypal')
        {
            ...
            if ($validator->fails()) {
                ...
                return redirect(Config::get('adminPrefix').'/settings/payment_methods');
            }
        }
    ...
    public function enableWoocommerce(Request $request)
    {
        ...
        $plugin_info = Setting::where(['name' => 'plugin_info'])->first();
        $wooCommerce = $this->helper->key_value('name', 'value', $wooCommerce);

        if ($request->method() != 'POST')
        {
            $data['menu'] = 'settings';
            $data['settings_menu']     = 'enablewoocommerce';
            ...
            $data['plugin_info']       = isset($plugin_info->value) ? json_decode($plugin_info->value) : [];
            ...
            if ($request->key == 'purchasecodeverification')
            {
                ...
                if ($purchaseStatus == 1)
                {
                    try
                    {
                        ...
                        return redirect(Config::get('adminPrefix').'/settings/enable-woocommerce');
                    }
                    catch (Exception $e)
                    {
                        DB::rollBack();
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        if ($_FILES["plugin"]["error"] == 0)
                        {
                            ...
                            if ($ext == 'zip')
                            {
                                if ($this->generteWcPluginBrand($request))
                                {
                        ...
                        else
                        {
                            ...
                            $fileSetting->type  = 'envato';
                            $fileSetting->save();
                            $this->generteWcPluginBrand($request);
                            DB::commit();
                            $this->helper->one_time_message('success', 'Plugin Uploaded Successfully');
                            return redirect(Config::get('adminPrefix').'/settings/enable-woocommerce');
                        }
    ...
    public function generteWcPluginBrand($request)
    {
        ...
        if (isset($request->plugin)) {
            ...
            $zipped_file_name = $request->plugin->getClientOriginalName();
            $zipped_file_name = pathinfo($zipped_file_name, PATHINFO_FILENAME);
        } else {
            $wooCommerce = Setting::where(['type' => 'envato'])->get(['value', 'name'])->toArray();
            $wooCommerce = $this->helper->key_value('name', 'value', $wooCommerce);
            $plugin = public_path('uploads/woocommerce/' . $wooCommerce['plugin_name']);
            $zipped_file_name = pathinfo($plugin, PATHINFO_FILENAME);
        }
        ...
        if ($res === true) {
            $res = $zip->extractTo(base_path($dir . '/' . $random_dir));
            $zip->close();
        } else {
            dd('could not open');
        }

        $WpStubFile = base_path($dir . '/' . $random_dir . '/src/main.stub');
        $WpMainFile = base_path($dir . '/' . $random_dir . '/woocommerce-paywallet.php');
        ...
        $replaceKey = [
            isset($request->plugin_name) ? $request->plugin_name : 'PayMoney - WooCommerce Payment Gateway',
            isset($request->plugin_uri) ? $request->plugin_uri : 'https://paymoney.techvill.org/',
            isset($request->plugin_author) ? $request->plugin_author : 'PayMoney developer Team',
            isset($request->plugin_author_uri) ? $request->plugin_author_uri : 'https://paymoney.techvill.org/',
            isset($request->plugin_base_url) ? $request->plugin_base_url : 'https://paymoney.techvill.org/',
            isset($request->plugin_description) ? $request->plugin_description : 'PayMoney payment gateway for Woocommerce',
        ...
        $rootPath = realpath($dir . '/' . $random_dir);

        // Initialize archive object
        $zipped = new ZipArchive();
        $zipped->open('public/uploads/woocommerce/'. $zipped_file_name .'.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE);
        ...
        \File::deleteDirectory($dir . '/' . $random_dir);

        $setting = Setting::updateOrCreate(['name' => 'plugin_info'], ['value' => json_encode($replaceKey)]);
