...
class LoginController extends Controller
{
    ...
    public $unverifiedEmailUser     = 202;
    public $unverifiedSmsUser       = 203;
    ...
    public function login(Request $request)
    {
        ...
        $checkPhoneVerificationStatus = $this->checkPhoneVerificationStatusApi($request->email);
        $user = User::where(['email' => $request->email])->first();
        $user_id = $user->id;
        if (($checkUserVerificationStatus == true)&&($checkPhoneVerificationStatus == true)) {
        ...
        {
            $success['status']  = $this->unverifiedEmailUser;
            $success['message'] = "Nous vous avons envoyé un code d'activation. Vérifiez votre e-mail et cliquez sur le lien pour vérifier.";

            return response()->json(['response' => $success], $this->unverifiedEmailUser);
        }
        if ($checkPhoneVerificationStatus == true)
        {
            $success['status']  = $this->unverifiedSmsUser;
            $success['message'] = 'We sent you an activation code. Check your phone and enter the code to verify.';
            $success['user_id'] =  $user->id;  

            return response()->json(['response' => $success], $this->unverifiedSmsUser);
        }
        else
        {
            //Auth attempt - starts
    ...
    public static function checkPhoneVerificationStatusApi($userEmail)
    {
        $user = User::where(['email' => $userEmail])->first();
        // if ($user->user_detail->phone_verified ==0 && $user->user_detail->phone_verification_code == 0 )
        if ($user->user_detail->phone_verified == 0 )

        // if ($user->user_detail->phone_verification_code == 0)
        {
            try{
                
                return RegistrationController::sendPhoneCode($user->id);
