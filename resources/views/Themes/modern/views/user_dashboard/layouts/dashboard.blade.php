                    ...
                    <div class="col-md-7 border-right p-4">
                        ...
                        @if (session('login') == 'success')
                            ...
                            count(
                                ...
                                : __('Welcome Back') }},
                                <span
                                    class="text-primary">{{ ' ' . auth()->user()->first_name . ' ' . auth()->user()->last_name }}</span>
                            </h2>
                        @else
                            @if (!empty($lastTransaction))
                                <h5>{{ __('Your last transaction was') }} <span
                                        class="text-primary">{{ number_format($lastTransaction->subtotal, '2', '.', '') }}
                                        {{ $lastTransaction->currency->code }} </span> <span style="font-size:small;">(
                                        {{ $transactionName }} )</span> </h5>
                            @else
                                @if ((count(auth()->user()->activity_log()->get()) == 1 ||
                                count(auth()->user()->activity_log()->get()) < 1)) <h2>{{ __('Howdy') }}, <span
                                        class="text-primary">{{ ' ' . auth()->user()->first_name . ' ' . auth()->user()->last_name }}</span>
                                    </h2>
                                @else
                                    <h2><span
                        ...
                        @endif
                        @endif
                        <p class="wel-text">{{ __('Thanks for using') }} <span
                                class="text-primary">{{ getCompanyName() }} </span>{{ __('services') }}</p>
                    </div>
                                                ...
                                                <thead>
                                                    <tr>
                                                        <th class="pl-5" scope="col">
                                                            @lang('message.dashboard.left-table.date')</th>
                                                        <th scope="col">@lang('message.dashboard.left-table.description')
                                                        </th>
                                                        <th scope="col">@lang('message.dashboard.left-table.status')</th>
                                                        <th class="text-right pr-5" scope="col">
                                                            @lang('message.dashboard.left-table.amount')</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @if ($transactions->count() > 0)
                                                        @foreach ($transactions as $key => $transaction)
                                                            <tr click="0" data-toggle="modal"
                                                                data-target="#collapseRow{{ $key }}"
                                                                aria-expanded="false"
                                                                aria-controls="collapseRow{{ $key }}"
                                                                class="show_area cursor-pointer"
                                                                trans-id="{{ $transaction->id }}"
                                                                id="{{ $key }}">
                                                                <td class="pl-5">
                                                                    <p class="font-weight-600 text-16 mb-0">
                                                                        {{ $transaction->created_at->format('jS F') }}
                                                                    </p>
                                                                    <p class="td-text">
                                                                        {{ $transaction->created_at->format('Y') }}</p>
                                                                </td>
                                                                    ...
                                                                    @if (!empty($transaction->end_user_id))
                                                                        <td class="text-left">
                                                                            @if ($transaction->transaction_type_id)
                                                                                ...
                                                                                @elseif($transaction->transaction_type_id
                                                                                    == Transferred)
                                                                                    <p class="text-16 mb-0">
                                                                                        {{ $transaction->end_user->first_name . ' ' . $transaction->end_user->last_name }}
                                                                                    </p>
                                                                                ...
                                                                                @elseif($transaction->transaction_type_id
                                                                                    ...
                                                                                    <p class="td-text">
                                                                                        @lang('Received')</p>
                                                                                @else
                                                                                    <p>{{ __(str_replace('_', ' ', $transaction->transaction_type->name)) }}
                                                                                    </p>
                                                                                @endif
                                                                            @endif
                                                                        </td>
                                                                    @else
                                                                        ...
                                                                        <td class="text-left">
                                                                            <p class="text-16 mb-0">
                                                                                ...
                                                                                @elseif($transaction->transaction_type->name
                                                                                    == 'Exchange_To' ||
                                                                                ...
                                                                                @elseif($transaction->transaction_type->name
                                                                                    ...
                                                                                    'unregistered')
                                                                                    {{ $transaction->email ? $transaction->email : $transaction->phone }}
                                                                                    <!--for send money by phone - mobile app-->
                                                                                @elseif($transaction->transaction_type->name
                                                                                    == 'Request_From' &&
                                                                                    $transaction->user_type =
                                                                                    'unregistered')
                                                                ...
                                                                <!--Status -->
                                                                <td>
                                                                    <span id="status_{{ $transaction->id }}"
                                                                        class="badge {{ $transaction->status }}">
                                                                ...
                                                                @if ($transaction->transaction_type_id == Deposit)
                                                                    @if ($transaction->subtotal > 0)
                                                                        <td class="text-right pr-5">
                                                                            <p><span class="text-16 font-weight-600">
                                                                                    +{{ formatNumber($transaction->subtotal) }}</span>
                                                                                <span
                                                                                    class="c-code">({{ $transaction->currency->code }})</span>
                                                                            </p>
                                                                        </td>
                                                                    @endif
                                                                ...
                                                                @elseif($transaction->transaction_type_id ==
                                                                    ...
                                                                    @if ($transaction->subtotal > 0)
                                                                        @if ($transaction->status == 'Refund')
                                                                            <td class="text-right pr-5">
                                                                                <p><span
                                                                                        class="text-16 font-weight-600">-{{ formatNumber($transaction->subtotal) }}</span>
                                                                                    <span
                                                                                        class="c-code">({{ $transaction->currency->code }})</span>
                                                                                </p>
                                                                    ...
                                                                    @elseif($transaction->subtotal < 0) <td
                                                                ...
                                                                @else
                                                                    @if ($transaction->total > 0)
                                                                        <td class="text-right pr-5">
                                                                            <p> <span class="text-16 font-weight-600">
                                                                                    {{ $transaction->currency->type != 'fiat' ? '+' . $transaction->total : '+' . formatNumber($transaction->total) }}
                                                                                </span> <span
                                                                                    class="c-code">({{ $transaction->currency->code }})</span>
                                                                            </p>
                                                                        </td>
                                                                    ...
                                                                    @elseif($transaction->total < 0) <td
                                                                            class="text-right pr-5">
                                                                            <p><span class="text-16 font-weight-600">{{ $transaction->currency->type != 'fiat' ? $transaction->total : formatNumber($transaction->total) }}
                                                                                </span> <span
                                                                                    class="c-code">({{ $transaction->currency->code }})</span>
                                                                            </p>
                                                                            </td>
                                                                ...
                                                                <div class="modal-dialog modal-lg modal-dialog-centered"
                                                                    ...
                                                                    <div class="modal-content">
                                                                        <div class="modal-body p-0">
                                                                            <button type="button"
                                                                                class="close text-28 pr-4 mt-2"
                                                                                data-dismiss="modal" aria-label="Close">
                                                                                <span aria-hidden="true">&times;</span>
                                                                            </button>

                                                                            <div class="row activity-details"
                                                                                id="loader_{{ $transaction->id }}"
                                                                                ...
                                                                                <div
                                                                                    class="col-md-7 col-sm-12 text-left p-0">
                                                                                    <div class="preloader transaction-loader"
                                                                                        style="display: none;">
                                                                                        <div class="loader"></div>
                                                                                    </div>

                                                                                    <div class="modal-header">
                                                                                        <h3 class="modal-title text-18 font-weight-600"
                                                                                    ...
                                                                                    <div class="col-md-12 col-sm-12 mt-5">
                                                                                        <div class="text-center mb-2">
                                                                                            @if ($transaction->transaction_type_id == Payment_Sent && $transaction->status == 'Success' && !isset($transaction->dispute->id))
                                                                                                <a id="dispute_{{ $transaction->id }}"
                                                                                                    href="{{ url('/dispute/add/') . '/' . $transaction->id }}"
                                                                                                    class="btn btn-primary btn-sm">@lang('message.dashboard.transaction.open-dispute')</a>
                                                                                            @endif
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                    ...
                                                    @else
                                                        <tr>
                                                            <td colspan="6" class="text-center p-4">
                                                                <img src="{{ theme_asset('public/images/banner/notfound.svg') }}"
                                                                    alt="notfound">
                                                                <p class="mt-4">{{ __('Sorry!') }}
                                                                    @lang('message.dashboard.left-table.no-transaction')</p>
                                                            </td>
                                                        </tr>
                                                    @endif
                                                </tbody>
                    ...
                    <div class="col-lg-4 mt-4">
                        ...
                        <div class="row">
                            <div class="col-md-12 mt-3">
                                <div class="shadow rounded bg-secondary pt-4 pb-4 pr-3 pl-3">
                                        ...
                                        <div class="w-100">
                                            @if ($wallets->count() > 0)
                                                @foreach ($wallets as $wallet)
                                                    ...
                                                    @endphp
                                                <div class="row">
                                                    <div class="wallet-balance col-9 d-flex align-items-center pl-3">
                                                            @if ($wallet->balance > 0)
                                                            ...
                                                            @elseif($wallet->balance == 0)
                                                                ...
                                                                @else
                                                                    <span>{{ '+' . formatNumber($wallet->balance) }}</span>
                                                                @endif
                                                            @elseif($wallet->balance < 0) @if ($wallet->currency->type != 'fiat')
                                                                <span>{{ $wallet->balance }}</span>
                                                            @else
                                                                <span>{{ '+' . formatNumber($wallet->balance) }}</span>
                                                            @endif
                                                            ...
                                                            @if ($wallet->currency->type == 'fiat' && $wallet->is_default == 'Yes')
                                                                <span>{{ $wallet->currency->code }}&nbsp;<span
                                                                        class="badge badge-secondary">@lang('message.dashboard.right-table.default-wallet-label')</span></span>
                                                            @else
                                                                <span>{{ $wallet->currency->code }}</span>
                                                            @endif
                                                    </div>
                                                    <div class="wallet-identifier col-3">
                                                        <button class="btn btn-link" onclick="setClipboardData('{{ $wallet->wallet_number }}')"
                                                                data-clipboard-target="#wallet-display-number">
                                                            <i class="fas fa-clipboard ml-1" data-toggle="tooltip"
                                                               data-placement="top"
                                                               title="{{ getInstitutioncode() . $wallet->wallet_number }}"></i>
                                                        </button>
                                                    </div>
                                                </div>
                                                @endforeach
